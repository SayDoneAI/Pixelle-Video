...
class ApiMediaService:
    ...
    async def _generate_image(
        self, prompt: str, width: Optional[int], height: Optional[int],
    ) -> MediaResult:
        """Call OpenAI-compatible image generation endpoint."""
    ...
    async def _poll_video_task(self, task_id: str) -> tuple[str, Optional[float]]:
        ...
        async with httpx.AsyncClient(timeout=httpx.Timeout(connect=10.0, read=60.0, write=10.0, pool=10.0)) as client:
            while time.monotonic() < deadline:
                ...
                data = await self._request_with_retry(url, headers, method="GET", client=client)
                status = data.get("status") or (data.get("data", {}) or {}).get("status") or ""
                status = status.lower()
                elapsed = _POLL_TIMEOUT - (deadline - time.monotonic())
    ...
    async def _request_with_retry(
        self, url: str, headers: dict, body: Optional[dict] = None,
        method: str = "POST", client: Optional[httpx.AsyncClient] = None,
    ) -> dict:
